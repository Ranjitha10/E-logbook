      ...
      <div class="navigation" id="sssignup">
        ...
        <p><?php
            $con=mysqli_connect("localhost", "root", "","project")or die("cannot connect");
              ...
              if(isset($_SESSION['host_success_sid']))
              $sid=$_SESSION['host_success_sid'];
              $qry= "SELECT * FROM Session WHERE sid='$sid'";
              $result=mysqli_query($con,$qry);

              if($result) {
              if(mysqli_num_rows($result) <= 0) {
                echo "<p> No session is active right now. Please host one. </p>"; 
              }
              else {  
              $row=mysqli_fetch_array($result);
                //echo "<p class='ui green leaf image'>";
                echo "Hosted successfully</p>";
                
                echo "<br />";
            
              echo "<table><thead><tr><th>Sid</th><th>Status</th><th>Subject</th><th>Batch</th><th>Experiment_no</th><th>Status</th></tr></thead><tbody><td>";
            echo $sid;
            echo "</td><td><i class='icon checkmark'></i> Session Created </td><td>";
              echo $row['sub'];
            echo "</td><td>";
            echo $row['batch'];
            echo "</td><td>";
            echo $row['experiment'];
            echo "</td><td><i class='icon checkmark'></i> Successful </td><td></tr></tbody></table>";
