...
if(empty($value1)||empty($value2)||empty($value3)||empty($value6)||empty($value7)||empty($value8)||empty($value9))
{
      ...
      if(preg_match($reg,$value7))
        {
          if(strlen($value6)==10)
           {
              $query="SELECT username from facultysignup";
              $result2=mysql_query($query);
              while($field_db=mysql_fetch_assoc($result2))
            {
                if($field_db['username']==$value3)
                    {
                      echo "<h3>Username already exists please enter another name</h3>";
                      //mysql_close($con_db);
                    }
            }
          if($value9==$value8)
            {
                  $result ="INSERT INTO facultysignup(fname,lname,username,emailid,contactno,password1,verify) VALUES ('$value1','$value2','$value3','$value6','$value7','$value8','$value9')";
